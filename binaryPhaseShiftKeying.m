...
axis([0 n lowlogiclevelvoltage-1
highlogiclevelvoltage+1]);
xlabel('Time(sec)');
ylabel('Amplitude(volts)');title('Message Signal');
...
axis([-0.05 n+0.05 lowlogiclevelvoltage-1
highlogiclevelvoltage+1]);
xlabel('Time(sec)');
...
phasecomponentoforsymbolzero = pi;
amplitudecomponet = 1;
...
modulatedsignal = amplitudecomponet*Ac.*sin(2*pi*fc*t+phasecomponent);
subplot(3,1,3);
